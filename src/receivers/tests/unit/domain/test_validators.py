...
from receivers.domain.validators import (
    validate_cpf,
    validate_cnpj,
    validate_cpf_cnpj,
    validate_email,
    validate_phone,
    validate_pix_random_key,
    validate_pix,
)
from receivers.tests.sample_values import (
    VALID_PHONE_LIST,
    INVALID_EMAIL_LIST,
    VALID_EMAIL_LIST,
    INVALID_CNPJ_LIST,
...
@pytest.mark.parametrize("phone", VALID_PHONE_LIST)
def test_validate_valid_phone(phone):
    validate_phone(phone)


@pytest.mark.parametrize("phone", INVALID_PHONE_LIST)
...
@pytest.mark.parametrize("pix_random_key", VALID_PIX_RANDOM_KEY_LIST)
def test_validate_valid_pix_random_key(pix_random_key):
    validate_pix_random_key(pix_random_key)


@pytest.mark.parametrize("pix_random_key", INVALID_PIX_RANDOM_KEY_LIST)
def test_validate_valid_pix_random_key(pix_random_key):
    with pytest.raises(ValueError) as error:
        validate_pix_random_key(pix_random_key)
